...
export function OnboardingWizard({
  ...
  const runFinish = async () => {
    if (!canLaunch) {
      toast.error("Name and market area are required");
      return;
    }
    setSyncing(true);
      ...
      <div className="relative w-full max-w-xl">
        <div className="mb-6 flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-[var(--color-fg-muted)]">
              One screen — name, market, website, and MLS. Then your workspace.
            </p>
          </div>
        </div>
        ...
        <div className="glass-panel surface-shine p-5 sm:p-6">
          <div className="mb-5 flex gap-2 rounded-[var(--radius-md)] border border-[color-mix(in_oklab,var(--color-primary)_30%,transparent)] bg-[color-mix(in_oklab,var(--color-primary-soft)_80%,transparent)] px-3 py-2 text-xs leading-relaxed text-[var(--color-fg-muted)]">
            ...
            <span>
              You’re signed in. Add the rest once — website is optional but pulls
              photo, phone, and listings when available. Never fake sample data.
            </span>
          </div>
          ...
          <p className="mt-4 text-center text-[11px] text-[var(--color-fg-subtle)]">
            Name + market required · everything else optional · edit anytime
          </p>
        </div>
      </div>
    </div>
