...
export function applyMemorySignal(
  ...
  const next: AgentMemory = {
    ...
    neighborhoods: { ...memory.neighborhoods },
    contentGoals: { ...memory.contentGoals },
    platforms: { ...memory.platforms },
    voices: { ...memory.voices },
    clientTags: { ...memory.clientTags },
    channels: { ...memory.channels },
    priorityKinds: { ...memory.priorityKinds },
    brandPhrases: [...memory.brandPhrases],
...
export function memoryInsights(
  ...
  insights.push(
    `${name}'s AI familiarity is ${memory.familiarityScore}/100 after ${memory.totalInteractions} interactions.`,
  );
  const n = topKeys(memory.neighborhoods, 3);
  if (n.length) insights.push(`Strongest market focus: ${n.join(" · ")}.`);
  const t = topKeys(memory.topics, 3);
  if (t.length) insights.push(`Recurring workstreams: ${t.join(" · ")}.`);
  if (memory.preferredVoice)
    insights.push(`Preferred brand voice: ${memory.preferredVoice}.`);
  insights.push(`Reply style locked to: ${memory.responseStyle}.`);
  const goals = topKeys(memory.contentGoals, 2);
  if (goals.length)
    insights.push(`Content goals you run most: ${goals.join(", ")}.`);
  const tags = topKeys(memory.clientTags, 3);
  if (tags.length)
    insights.push(`Client tags you touch most: ${tags.join(", ")}.`);
  for (const f of memory.learnedFacts.slice(0, 4)) {
...
export function personalizationPreamble(
  ...
  const n = topKeys(memory.neighborhoods, 2);
  const facts = memory.learnedFacts.slice(0, 3).map((f) => f.text);
  const bits = [
    ...
    `Familiarity ${memory.familiarityScore}/100`,
  ].filter(Boolean);
  return bits.join(" · ");
}
